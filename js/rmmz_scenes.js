...
Scene_Base.prototype.isReady = function() {
    const imageReady = ImageManager.isReady();
    const effectReady = EffectManager.isReady();
    const fontReady = FontManager.isReady();
    const allReady = imageReady && effectReady && fontReady;
...
Scene_Map.prototype.initialize = function() {
    ...
    this._readyCheckTimeout = 300; // 5 seconds at 60fps
    this._encounterEffectStuckCount = 0;
    this._encounterEffectStuckTimeout = 180; // 3 seconds at 60fps
};
...
Scene_Map.prototype.isReady = function() {
    ...
    let ready = this._mapLoaded && Scene_Message.prototype.isReady.call(this);

    // Safety timeout: force ready if stuck too long
    if (!ready && this._mapLoaded) {
        this._readyCheckCount++;
        if (this._readyCheckCount >= this._readyCheckTimeout) {
            ready = true;
        }
    } else if (ready) {
        this._readyCheckCount = 0;
...
Scene_Map.prototype.isBusy = function() {
    const messageClosing = this.isMessageWindowClosing();
    const waitCount = this._waitCount > 0;
    let encounterEffect = this._encounterEffectDuration > 0;
    const parentBusy = Scene_Message.prototype.isBusy.call(this);

    // Safety timeout: force clear stuck encounter effects
    if (encounterEffect) {
        this._encounterEffectStuckCount++;
        if (this._encounterEffectStuckCount >= this._encounterEffectStuckTimeout) {
            this._encounterEffectDuration = 0;
    ...
    const busy = messageClosing || waitCount || encounterEffect || parentBusy;
...
Scene_Map.prototype.startEncounterEffect = function() {
    ...
    this._encounterEffectStuckCount = 0; // Reset stuck counter
};
...
Scene_Menu.prototype.commandPersonal = function() {
    const symbol = this._commandWindow.currentSymbol(); // Get the selected command's symbol
    const actor = $gameParty.members()[0]; // Always use the first actor

    switch (symbol) {
        case "skill":
            SceneManager.push(Scene_Skill); // Go directly to Skills
            break;
        case "equip":
            SceneManager.push(Scene_Equip); // Go directly to Equip
            break;
        case "status":
            SceneManager.push(Scene_Status); // Go directly to Status
            break;
    }
/*
    SceneManager.prepareNextScene = function() {
        SceneManager._scene.setActor(actor); // Set the first actor for each screen
    };*/
};
...
Scene_Load.prototype.executeLoad = function(savefileId) {
    console.log("Scene_Load.executeLoad - Starting load for savefileId:", savefileId);
    DataManager.loadGame(savefileId)
        .then(() => {
            console.log("Load promise resolved successfully");
            this.onLoadSuccess();
        })
        .catch((error) => {
            console.error("Load promise rejected - calling onLoadFailure");
...
Scene_Load.prototype.onLoadFailure = function(error) {
    console.error("=== LOAD FAILURE - BUZZER SOUND ===");
    ...
    if (error) {
        ...
        console.error("No error object provided to onLoadFailure");
    }
    console.error("====================================");
...
Scene_Battle.prototype.logWindowRect = function() {
    const wx = 0;
    const ww = Graphics.boxWidth;
    const wh = this._logWindowHeight ? this._logWindowHeight() : this.calcWindowHeight(4, true);
    const wy = Graphics.boxHeight - wh; // Position at bottom
    return new Rectangle(wx, wy, ww, wh);
};
...
Scene_Battle.prototype.createHelpWindow = function() {
    ...
    this.addWindow(this._helpWindow);
    this._helpWindow.y += 250

};

Scene_Battle.prototype.helpWindowRect = function() {
    const wx = 0;
    const wy = 60; // <-- Move it 100px down
    const ww = Graphics.boxWidth;
    const wh = this.calcWindowHeight(2, false); // 2 rows high
    return new Rectangle(wx, wy, ww, wh);
};
...
Scene_Battle.prototype.createSkillWindow = function() {
    ...
    const wy = 0;
    const ww = Graphics.boxWidth;
    const wh = Graphics.boxHeight - 100; // leave space for description
    const skillWindowRect = new Rectangle(wx, wy, ww, wh);

    this._skillWindow = new Window_BattleSkill(skillWindowRect);
    this._skillWindow.setHelpWindow(this._helpWindow);
    this._skillWindow.setHandler("ok", this.onSkillOk.bind(this));
    this._skillWindow.setHandler("cancel", this.onSkillCancel.bind(this));
    this.addWindow(this._skillWindow);

    // Reposition HelpWindow
    ...
    this._helpWindow.setText(""); // Clear on creation
};
...
Scene_Battle.prototype.createItemWindow = function() {
    ...
    const ww = Graphics.boxWidth;
    const wh = Graphics.boxHeight - 100; // leave space for description
    const itemWindowRect = new Rectangle(wx, wy, ww, wh);

    this._itemWindow = new Window_BattleItem(itemWindowRect);
    this._itemWindow.setHelpWindow(this._helpWindow);
    this._itemWindow.setHandler("ok", this.onItemOk.bind(this));
    this._itemWindow.setHandler("cancel", this.onItemCancel.bind(this));
    this.addWindow(this._itemWindow);
...
Scene_Battle.prototype.startPartyCommandSelection = function() {
    this.selectNextCommand(); // Skip "Fight / Escape", go straight to actor commands
