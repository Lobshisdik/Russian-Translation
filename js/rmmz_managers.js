...
DataManager.loadGame = function(savefileId) {
    console.log("=== SAVE LOAD DEBUG START ===");
    console.log("Loading save file ID:", savefileId);
    const saveName = this.makeSavename(savefileId);
    console.log("Save file name:", saveName);

    return StorageManager.loadObject(saveName).then(contents => {
        console.log("✓ StorageManager.loadObject completed successfully");
        console.log("Contents received:", contents ? "Valid object" : "NULL/UNDEFINED");

        console.log("Creating game objects...");
        this.createGameObjects();
        console.log("✓ Game objects created");

        console.log("Extracting save contents...");
        this.extractSaveContents(contents);
        console.log("✓ Save contents extracted");

        console.log("Correcting data errors...");
        this.correctDataErrors();
        console.log("✓ Data errors corrected");

        console.log("=== SAVE LOAD DEBUG SUCCESS ===");
        return 0;
    }).catch(error => {
        console.error("=== SAVE LOAD DEBUG FAILED ===");
        console.error("Error during loadGame:", error);
...
DataManager.extractSaveContents = function(contents) {
    console.log("Extracting save contents - validating structure...");
    const requiredFields = ['system', 'screen', 'timer', 'switches', 'variables', 'selfSwitches', 'actors', 'party', 'map', 'player'];
    const missingFields = requiredFields.filter(field => !contents[field]);

    if (missingFields.length > 0) {
        console.error("MISSING REQUIRED FIELDS:", missingFields);
        console.error("Available fields:", Object.keys(contents));
    } else {
    ...
    console.log("Save extraction complete:");
    console.log("- Map ID:", $gameMap ? $gameMap.mapId() : "UNDEFINED");
    console.log("- Player position:", $gamePlayer ? `(${$gamePlayer.x}, ${$gamePlayer.y})` : "UNDEFINED");
    console.log("- Party size:", $gameParty ? $gameParty.size() : "UNDEFINED");
};
...
StorageManager.loadFromForage = function(saveName) {
    const key = this.forageKey(saveName);
    console.log("loadFromForage - Forage key:", key);

    return localforage.getItem(key).then(data => {
        console.log("localforage.getItem returned:", data ? `Data (${data.length} bytes)` : "NULL");
        return data;
    }).catch(error => {
        console.error("localforage.getItem FAILED:", error.message);
        throw error;
    });
};
...
SceneManager.isSceneChanging = function() {
    const changing = this._exiting || !!this._nextScene;
    return changing;
};

SceneManager.isCurrentSceneBusy = function() {
    const busy = this._scene && this._scene.isBusy();
    return busy;
};
...
SceneManager.pop = function() {
    if (this._stack.length > 0) {
        const previousScene = this._stack.pop();
...
BattleManager.selectNextCommand = function() {
    if (this._currentActor) {
        // If the current action is Attack, automatically target a random enemy
        const action = this._currentActor.inputtingAction();
        if (action && action.isAttack()) {
            ...
            if (aliveEnemies.length > 0) {
                ...
                const randomEnemyIndex = $gameTroop.members().indexOf(aliveEnemies[randomIndex]);
                action.setTarget(randomEnemyIndex);
            }
        }
...
BattleManager.processEscape = function() {
    console.log('[DEBUG] CORE BattleManager.processEscape CALLED');
    $gameParty.performEscape();
    SoundManager.playEscape();
    const success = this._preemptive || Math.random() < this._escapeRatio;
    console.log('[DEBUG] - escape success:', success, '(preemptive:', this._preemptive, ', escapeRatio:', this._escapeRatio, ')');
    if (success) {
        console.log('[DEBUG] - onEscapeSuccess will be called');
        this.onEscapeSuccess();
    } else {
        console.log('[DEBUG] - onEscapeFailure will be called');
        this.onEscapeFailure();
    }
    console.log('[DEBUG] CORE BattleManager.processEscape ENDED');
    return success;
};

BattleManager.onEscapeSuccess = function() {
    console.log('[DEBUG] BattleManager.onEscapeSuccess CALLED');
    this.displayEscapeSuccessMessage();
    this._escaped = true;
    console.log('[DEBUG] - _escaped set to true, calling processAbort');
    this.processAbort();
};
...
BattleManager.processAbort = function() {
    console.log('[DEBUG] BattleManager.processAbort CALLED');
    $gameParty.removeBattleStates();
    this._logWindow.clear();
    this.replayBgmAndBgs();
    console.log('[DEBUG] - calling endBattle(1) for escape');
    this.endBattle(1);
};

BattleManager.processDefeat = function() {
    console.log('[DEBUG] CORE BattleManager.processDefeat CALLED');
    console.log('[DEBUG] - canLose:', this._canLose);
    this.displayDefeatMessage();
    this.playDefeatMe();
    if (this._canLose) {
        console.log('[DEBUG] - Can lose, replaying BGM');
        this.replayBgmAndBgs();
    } else {
        console.log('[DEBUG] - Cannot lose, stopping BGM');
        AudioManager.stopBgm();
    }
    console.log('[DEBUG] - calling endBattle(2) for defeat');
    this.endBattle(2);
};

BattleManager.endBattle = function(result) {
    console.log('[DEBUG] CORE BattleManager.endBattle CALLED');
    console.log('[DEBUG] - result:', result, '(0=victory, 1=escape, 2=defeat)');
    console.log('[DEBUG] - _escaped:', this._escaped);
    this._phase = "battleEnd";
    console.log('[DEBUG] - phase set to: battleEnd');
    this.cancelActorInput();
    this._inputting = false;
    if (this._eventCallback) {
        console.log('[DEBUG] - Calling event callback with result:', result);
        this._eventCallback(result);
    }
    if (result === 0) {
        console.log('[DEBUG] - Victory: calling onBattleWin');
        $gameSystem.onBattleWin();
    } else if (this._escaped) {
        console.log('[DEBUG] - Escaped: calling onBattleEscape');
        $gameSystem.onBattleEscape();
    }
    $gameTemp.clearCommonEventReservation();
    console.log('[DEBUG] CORE BattleManager.endBattle COMPLETED');
};

BattleManager.updateBattleEnd = function() {
    console.log('[DEBUG] CORE BattleManager.updateBattleEnd CALLED');
    console.log('[DEBUG] - isBattleTest:', this.isBattleTest());
    console.log('[DEBUG] - _escaped:', this._escaped);
    console.log('[DEBUG] - party all dead:', $gameParty.isAllDead());
    console.log('[DEBUG] - _canLose:', this._canLose);

    if (this.isBattleTest()) {
        console.log('[DEBUG] - Battle test mode, exiting');
        AudioManager.stopBgm();
        SceneManager.exit();
    } else if (!this._escaped && $gameParty.isAllDead()) {
        console.log('[DEBUG] - Party defeated (not escaped)');
        if (this._canLose) {
            console.log('[DEBUG] - Can lose: reviving party and popping scene');
            $gameParty.reviveBattleMembers();
            SceneManager.pop();
        } else {
            console.log('[DEBUG] - Cannot lose: going to game over');
            SceneManager.goto(Scene_Gameover);
        }
    } else {
        console.log('[DEBUG] - Normal battle end: popping scene');
        SceneManager.pop();
    }
    this._phase = "";
    console.log('[DEBUG] - phase cleared, CORE BattleManager.updateBattleEnd COMPLETED');
};
